// ==UserScript==
// @name         Avito Transit Time via Yandex Maps (exact variant)
...
(function () {
  ...
  const POLL_MS = 700;
  const ROUTE_REQUEST_DELAY_MS = 2000;
  ...
  function getElementTextValue(element) {
    ...
    if ('value' in element && normalizeText(element.value)) return normalizeText(element.value);
    if (element.isContentEditable && normalizeText(element.textContent || '')) return normalizeText(element.textContent || '');
